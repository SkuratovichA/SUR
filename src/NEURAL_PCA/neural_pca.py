    ...
    def __init__(self, is_train: bool):
        if is_train:
            self.transform = transforms.Compose([
                iaa.Sequential([
                    iaa.Resize((80, 80)),
                    iaa.Sometimes(0.1, iaa.GaussianBlur(sigma=(0, 2.0))),
                    iaa.Sometimes(0.3, iaa.PerspectiveTransform(scale=(0.0, 0.03))),
                    iaa.Fliplr(0.8),
                    iaa.Sometimes(0.7, iaa.AddToHueAndSaturation(value=(-10, 10), per_channel=True)),
                    iaa.Sometimes(0.2,
...
class PCADataset(pl.LightningDataModule):
    def __init__(self, root_dir: str = '.', batch_size: int = 4):
        ...
        def pca_transform(datas, u, mean_):
            ...
            return d_u
        train_d, train_l = get_images(os.path.join(root_dir, 'train1.csv'), is_train=True)
        val_d, val_l = get_images(os.path.join(root_dir, 't_dev.csv'), is_train=False)  # t_dev.csv, nt_dev.csv, dev1.csv
...
class NeuralPCA(pl.LightningModule):
    def __init__(self, IN_SHAPE):
        ...
        self.model = nn.Sequential(
            ...
            nn.Linear(in_features=IN_SHAPE, out_features=64),
            self.activ2,
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.RMSprop(self.parameters(), lr=0.00000015, weight_decay=0.020, momentum=0.991)  # the best?
        #optimizer = torch.optim.Adam(self.parameters(), lr=0.000005, weight_decay=0.015, amsgrad=True)  # the best?
...
def main(hparams):
    ...
    data = PCADataset(root_dir=hparams["dataset_dir"], batch_size=2)
    ...
    if hparams["train"]:
        ...
        with open(dataset_personality, "wb") as f:
            ...
            np.save(file=f, arr=mean)
            np.save(file=f, arr=IN_SHAPE)
    ...
    if True:
        ...
        imgs = ["../../dataset/target_dev/m421_03_p01_i0_0.png", "../../dataset/non_target_dev/f408_05_r07_i0_0.png"]
...
if __name__ == "__main__":
    ...
    hparams = {"train": True,
               "model_dir": "../models",
               "model_name": "Neural_PCA_GELU_tmp.pt",
               "root_dir": "../",
