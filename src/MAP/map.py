...
class Classifier:
    ...
    def __init__(self, hparams):
        ...
        # CREATING DATASETS
        if hparams["train"]:
            dataset_directory = self.hparams["dataset_dir"]
            csv = pd.read_csv(os.path.join(dataset_directory, 'train_all_map.csv'))
            target_filenames = []
            non_target_filenames = []
            for id in range(len(csv)):
                label = int(csv.iloc[id, 1])
                filename = os.path.join(dataset_directory, csv.iloc[id, 0])
                if label == 1:
                    target_filenames.append(filename)
                else:
                    non_target_filenames.append(filename)

            self.train_d = Dataset(self.hparams, filenames=target_filenames, aug=True)
            self.train_nd = Dataset(self.hparams, filenames=non_target_filenames, aug=True)
