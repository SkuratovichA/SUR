...
#    Tikhonov Maksim <xtikho00@vutbr.cz>
# Date: 27.4.2022
...
from NEURAL_PCA.neural_pca import NeuralPCA
    ...
    def __init__(self, hparams):
        ...
        if self.hparams["eval"]:
            with open(self.hparams["u_mean"], "rb") as f:
                self.u = np.load(f)
                self.mean = np.load(f)
                self.in_shape = np.load(f)
            path = os.path.join(hparams["model_dir"], hparams["model_name"])
            self.model = NeuralPCA(self.in_shape)
...
class CNNClassifier(Classifier):
    ...
    def predict(self, filename):
        ...
        image = transform(np.array(Image.open(filename).convert('RGB'))).unsqueeze(0)
        soft = self.model(image).detach().ravel()[0]
